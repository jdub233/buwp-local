...
function generateComposeConfig(config) {
  ...
  // Database service
  composeConfig.services.db = generateDbService(config);

  // WordPress service
  composeConfig.services.wordpress = generateWordPressService(config);
 ...
 */
function generateDbService(config) {
 ...
 */
function generateWordPressService(config) {
  const depends_on = ['db'];
  ...
  // Build volumes array
  const volumes = ['wp_build:/var/www/html'];
