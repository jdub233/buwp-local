...
async function updateCommand(options = {}) {
  ...
  try {
    ...
    // Step 2: Wipe wp_build volume (unless --preserve-wpbuild flag is used)
    // This ensures WordPress core files are updated from the new image
    // Database and custom mapped code are always preserved
    ...
    if (!preserveWpBuild) {
      ...
      try {
        // Remove wp_build volume (db_data is preserved)
        execSync(
        ...
        // Volume might not exist or might be in use - not critical
        console.log(chalk.gray('Note: WordPress volume will be recreated from image\n'));
      }
    } else {
    ...
    console.log(chalk.cyan('🔨 Recreating containers with new images...'));
