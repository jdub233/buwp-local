...
async function updateCommand(options = {}) {
  ...
  try {
    ...
    // Step 2: Stop and remove containers to release volumes (unless --preserve-wpbuild)
    // This is required before we can delete the wp_build volume
    // Note: We use 'down' without -v flag to preserve db_data volume
    const preserveWpBuild = options.preserveWpbuild || false;
    
    if (!preserveWpBuild) {
      console.log(chalk.cyan('\n🛑 Stopping containers to release volumes...'));
      try {
      ...
      // Step 3: Remove wp_build volume (db_data is preserved)
